...
return [
    ...
    'description' => 'API v1 for authentication (OTP), profile, and payments.',
    ...
    'auth' => [
        ...
        'extra_info' => 'Send a **POST** request to **/api/v1/authenticate** with your email to receive an OTP, then **POST** to **/api/v1/verify-otp** with `email` and `otp` to get your Bearer token. Use it in the header: `Authorization: Bearer <token>`.',
    ],
    ...
    // For response calls, Scribe will try to start database transactions so no changes are persisted.
    // Empty array = skip transactions (scribe:generate works when DB is unavailable; GETs don't mutate data).
